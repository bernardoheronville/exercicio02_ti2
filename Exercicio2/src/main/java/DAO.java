...
public class DAO {
	...
	public boolean conectar() {
		...
		String username = "bernardosql";
		String password = "bernardo123";
	...
	public boolean inserirAluno(Aluno aluno) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO aluno (codigo, nome, idade, nota) "
					       + "VALUES ("+aluno.getCodigo()+ ", '" + aluno.getNome() + "', '"  
					       + aluno.getIdade() + "', '" + aluno.getNota() + "');");
			st.close();
			status = true;
	...
	public boolean atualizarAluno(Aluno aluno) {
		...
		try {  
			Statement st = conexao.createStatement();
			String sql = "UPDATE aluno SET nome = '" + aluno.getNome() + "', idade = '"  
				       + aluno.getIdade() + "', nota = '" + aluno.getNota() + "'"
	         ...
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	            	 alunos[i] = new Aluno(rs.getInt("codigo"), rs.getString("nome"), 
	                		                  rs.getInt("idade"), rs.getDouble("nota"));
	             }
	          }
	          st.close();
		} catch (Exception e) {
